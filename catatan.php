...
$babies = $koneksi->execute_query($sql1)->fetch_assoc(MYSQLI_ASSOC);

if ($_SERVER ['REQUEST_METHOD'] === 'POST') {
    session_start();
    $id_kader = $_SESSION ['id_pengguna'];
    $id_bayi = $_SESSION ['bayi'];
    $tinggi = $_SESSION ['tinggi'];
    $berat = $_SESSION ['berat'];

    $sql = "INSERT INTO bayi (id_kader, id_bayi, tinggi, berat, tanggal) VALUES (?,?,?,?,CURRENT_DATE)";
    ...
    <form action="" method="post">
        <div class="form-item">
            ...
            <select name="bayi" id="bayi">
                <?php foreach ($babies as $bayi) : ?>
                <option value="<?=$baby['id _bayi']?>"><?=$baby['nama']?></option>
                <?php endforeach ?>
            </select>
        </div>
        <div class="form-item">
